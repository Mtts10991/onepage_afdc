...
const buttonVariants = cva(
  // base: hover/press feedback + cursor + smooth transitions ทุกตัว
    ...
    "[&_svg]:size-4 [&_svg]:shrink-0 [&_svg]:transition-transform [&_svg]:duration-150",
  {
    variants: {
      variant: {
        ...
        outline:
          "border border-input bg-background hover:bg-accent hover:text-accent-foreground hover:border-accent-foreground/30 hover:shadow-sm",
        secondary:
      ...
      size: {
        ...
        icon: "h-9 w-9 hover:[&_svg]:scale-110",
      },
    },
    defaultVariants: {
      variant: "default",
...
export interface ButtonProps
  ...
  asChild?: boolean;
  /**
   * While true the button is disabled and shows a spinner. Use this to
   * block double-clicks on async actions (delete, toggle, save) so a slow
   * API/Supabase response can't be triggered twice.
   */
  loading?: boolean;
}
